...
def binarize_all(clean_dir, bin_dir):
    ...
    for info in infos:
        ...
        for i in range(1, frame.shape[1]):
            ...
            cut_points = discretizer.bin_edges_[i - 1][1:-1] # Exclude label and left/right edges

            for j in range(len(cut_points)):
                frame[series_name + f"_bin_{j}"] = (series >= cut_points[j])*1
                print(f"Uniques {series_name}_bin_{j}: {len(frame[series_name + f'_bin_{j}'].unique())}, Sum: {frame[series_name + f'_bin_{j}'].sum()}")
            binarized.append(series_name)

        info["bins"] = list(map(lambda a: a.tolist(), discretizer.bin_edges_))
        frame.drop(columns=binarized, inplace=True)
